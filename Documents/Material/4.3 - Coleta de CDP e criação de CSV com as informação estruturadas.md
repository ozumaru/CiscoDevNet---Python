...
```python
            ...
            for device, ip_device, input_port, outgoing_port in zip(device, ip_device, input_port, outgoing_port):
                ...
                with open(f"Local do diretorio de output{hostname}_{now}.csv", mode="w", newline="", encoding="utf-8") as arquivo_csv:
...
- Ir para: 
    - Script sem comentário: [Coleta de CDP e criação de CSV com as informação estruturadas](Adicionar)
    - Inicio: [Python para Network](https://github.com/ozumaru/CiscoDevNet---Python)
    - Proximo assunto: 5.1 - Rotina de Backup - Emprogresso